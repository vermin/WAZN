# Anonymity Networks with WAZN

Currently only Tor and I2P have been integrated into WAZN. The usage of
...
is made or until `waznd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `wazn-wallet-cli` and
`wazn-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `waznd` the ip/port of a socks proxy provided by a
...
which tells `waznd` that ".onion" p2p addresses can be forwarded to a socks
...
```
--add-exclusive-node rveahdfho7wo4b2m.onion:22789
--add-peer rveahdfho7wo4b2m.onion:22789
```
...
`--anonymous-inbound`. This option tells `waznd` the inbound address, network
type, and max connections:

```
--anonymous-inbound rveahdfho7wo4b2m.onion:22789,127.0.0.1:22789,25
...
`waznd` RPC port - which is independent from the configuration for incoming
P2P anonymity connections. The anonymity network (Tor/i2p) is
[configured in the same manner](#configuration), except the localhost port
must be the RPC port (typically 11787 for Mainnet) instead of the p2p port:

```
HiddenServiceDir /var/lib/tor/data/wazn
...
```
HiddenServiceDir /var/lib/tor/data/wazn
HiddenServicePort 22789 127.0.0.1:22789
```

This will store key information in `/var/lib/tor/data/wazn` and will forward
"Tor port" 22789 to port 22789 of ip 127.0.0.1. The file
`/usr/lib/tor/data/wazn/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the WAZN network should have
similar timestamps.
...
### Intermittent WAZN Syncing

If a user only runs `waznd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `waznd` as often as possible to conceal when transactions are being sent.
...
`waznd` currently selects two outgoing connections every 5 minutes for
...
`waznd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `waznd` should likely disconnect
